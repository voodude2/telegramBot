...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (!redis) return res.status(500).json({ error: 'Redis is not configured' });
    ...
    const user = { id: userId, email, name, passwordHash, createdAt: new Date().toISOString() };
    
    await redis.set(`user:${email}`, JSON.stringify(user));
    await redis.set(`userId:${userId}`, email); // Reverse lookup
...
async function processAIChat({ sessionId, userMessage, platform = 'web', media = null, onChunk = null, userName = null }) {
  let systemInstruction = userName 
    ? `You are a friendly and knowledgeable AI consultant for TechStore. You are talking to a registered user named ${userName}. Greet them by name when appropriate and act as their personal shopping assistant. `
...
// Web API endpoint for Website AI Chat Widget (SSE Streaming)
  app.post('/api/chat', async (req, res) => {
    try {
      const { message, sessionId, media, userName } = req.body;
      if (!message && !media) {
      ...
      const result = await processAIChat({ sessionId: cleanSessionId, userMessage: message || "What is in this photo?", platform: 'web', media, onChunk, userName });
